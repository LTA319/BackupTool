...
/// <summary>
/// 传输日志查看器控件
/// Transfer log viewer control
/// </summary>
public partial class TransferLogViewerControl : UserControl, IEmbeddedForm
{
    #region 私有字段
    ...
    private readonly ILogger<TransferLogViewerControl> _logger;
    ...
    /// 获取嵌入式窗体的显示标题
    /// </summary>
    public string Title => "传输日志查看器";

    /// <summary>
    /// 获取导航路径用于面包屑显示
    /// </summary>
    public string NavigationPath => "工具 > 传输日志查看器";
    ...
    public void OnActivated()
    {
        try
        {
            ...
            _logger.LogInformation("传输日志查看器控件已激活");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "激活传输日志查看器控件时发生错误");
        }
    }
    ...
    public void OnDeactivated()
    {
        try
        {
            _logger.LogInformation("传输日志查看器控件已停用");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "停用传输日志查看器控件时发生错误");
        }
    }
    ...
    public TransferLogViewerControl(IServiceProvider serviceProvider)
    {
        _serviceProvider = serviceProvider;
        _transferLogService = serviceProvider.GetRequiredService<ITransferLogService>();
        _logger = serviceProvider.GetRequiredService<ILogger<TransferLogViewerControl>>();
    ...
    public void SetBackupLogId(int backupLogId)
    {
        ...
        var title = $"传输日志查看器 - 备份 #{backupLogId}";
    ...
    private void HideProgress()
    {
        ...
        StatusChanged?.Invoke(this, "准备就绪");
        this.Enabled = true;
    }
